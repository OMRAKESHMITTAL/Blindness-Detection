...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        y_val = self.validation_data[1].sum(axis=1) - 1

        y_pred = self.model.predict(self.validation_data[0]) > 0.5
...
# Using original generator
data_generator = create_datagen().flow(x_train, y_train, batch_size=BATCH_SIZE, seed=2019)
val_data = (x_val, y_val)  # Prepare validation data
...
history = model.fit(
    ...
    epochs=15,
    validation_data=val_data,  # Use the validation data here
